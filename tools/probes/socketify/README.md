...
```sh
git clone https://github.com/cirospaciari/socketify.py.git build/socketify-source
git -C build/socketify-source checkout d35e41921dadcd8e27679b7300b51e0a697bfcba
```
...
& $python -m pip install --only-binary=:all: --target build/socketify-deps-windows cffi==2.1.1 pycparser==3.0 setuptools==84.0.0
$env:PYTHONPATH = "$PWD/build/socketify-source/src;$PWD/build/socketify-deps-windows"
$env:OOTH_TEST_SOCKETIFY = $python
$env:CGO_ENABLED = '0'
./build/windows-amd64/go/bin/go.exe test -v -run '^TestSocketifyExtraHandle$' .
```

Linux:

```sh
python3 -m pip install --only-binary=:all: --target build/socketify-deps-linux cffi==2.1.1 pycparser==3.0 setuptools==84.0.0
PYTHONPATH="$PWD/build/socketify-source/src:$PWD/build/socketify-deps-linux" \
OOTH_TEST_SOCKETIFY=python3 CGO_ENABLED=0 \
  ./build/linux-amd64/go/bin/go test -v -run '^TestSocketifyExtraHandle$' .
```
...
```sh
mkdir -p build/socketify-libuv
(cd build/socketify-libuv && apt download libuv1t64=1.48.0-1.1build1)
dpkg-deb -x build/socketify-libuv/libuv1t64_1.48.0-1.1build1_amd64.deb build/socketify-libuv
export LD_LIBRARY_PATH="$PWD/build/socketify-libuv/usr/lib/x86_64-linux-gnu${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"
```
