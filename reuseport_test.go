		...
		for _, protocol := range protocols {
			for _, proxy := range []string{"direct", "caddy", "nginx"} {
				t.Run(name+"/"+protocol+"/"+proxy, func(t *testing.T) {
					...
					script := "tools/probes/node_worker.cjs"
					switch name {
					case "PYTHON":
						script = "tools/probes/reuseport/python.py"
					case "BUN", "DENO":
						script = "tools/probes/reuseport/native.mjs"
						if name == "DENO" {
							command = append(command, "run", "-A", "--unstable-net")
						}
					case "SOCKETIFY":
						script = "tools/probes/socketify/worker.py"
						workerEnv["TEST_OOTH_PROTOCOL"] = "1"
					case "TRUEASYNC":
						script = "tools/probes/trueasync/worker.php"
