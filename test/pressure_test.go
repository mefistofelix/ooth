...
func runPressure(t *testing.T, root string, scenario pressureCase) {
	...
	if scenario.Runtime != "" {
		...
		case "PYTHON", "NODE":
			...
			script, _ := filepath.Abs(filepath.Join("..", "test", "integration", "pressure", "worker."+extension))
...
func pressureProxy(t *testing.T, directory, backend, proxy string) string {
	...
	script, err := os.ReadFile(filepath.Join("..", "test", "integration", "pressure", "worker.php"))
	if err != nil {
		t.Fatal(err)
	}
...
func pressureFastCGI(request *pressureRequest, connection net.Conn) {
	...
	script, _ := filepath.Abs(filepath.Join("..", "test", "integration", "pressure", "worker.php"))
