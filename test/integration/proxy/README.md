...
```powershell
./test/integration/proxy/prepare-windows.ps1
$env:CGO_ENABLED = '0'
$env:OOTH_TEST_PYTHON = 'C:/path/to/python.exe'
$env:OOTH_TEST_NODE = 'C:/path/to/node.exe'
$env:OOTH_TEST_CADDY = "$PWD/tmp/runtime-proxy/caddy-windows/caddy.exe"
$env:OOTH_TEST_NGINX = "$PWD/tmp/runtime-proxy/nginx-1.30.5/nginx.exe"
$env:OOTH_TEST_PHP_CGI = "$PWD/tmp/runtime-proxy/php-windows/php-cgi.exe"
$env:OOTH_TEST_PHP_LAUNCHER = "$PWD/tmp/runtime-proxy/php-worker.exe"
$env:OOTH_TEST_ARTIFACTS = "$PWD/tmp/proxy-results-windows"
./build/windows-amd64/go/bin/go.exe run ./test/run.go test -run 'TestProxyStack|TestStockPHPWindowsInheritedDetection' -v -timeout 90s .
```

## Reproduce on Linux

```sh
bash test/integration/proxy/prepare-linux.sh
export CGO_ENABLED=0
export OOTH_TEST_PYTHON=/usr/bin/python3
export OOTH_TEST_NODE=/absolute/path/to/node
export OOTH_TEST_CADDY="$PWD/tmp/runtime-proxy/caddy-linux/caddy"
export OOTH_TEST_NGINX="$PWD/tmp/runtime-proxy/nginx-linux/objs/nginx"
export OOTH_TEST_PHP_CGI="$PWD/tmp/runtime-proxy/php-linux/usr/bin/php-cgi8.3"
export OOTH_TEST_ARTIFACTS="$PWD/tmp/proxy-results-linux"
./build/linux-amd64/go/bin/go run ./test/run.go test -run TestProxyStack -v -timeout 90s .
```
...
sudo --preserve-env=OOTH_TEST_PYTHON,OOTH_TEST_NODE,OOTH_TEST_CADDY,OOTH_TEST_NGINX,OOTH_TEST_PHP_CGI,OOTH_TEST_ARTIFACTS \
  bash test/integration/with-cgroup.sh "$USER" env CGO_ENABLED=0 \
  ./build/linux-amd64/go/bin/go run ./test/run.go test -run TestProxyStack -v -timeout 90s .
```
