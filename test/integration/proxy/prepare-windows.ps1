...
$destination = Join-Path $repository 'tmp/runtime-proxy'
...
$env:CGO_ENABLED = '0'
& (Join-Path $repository 'build/windows-amd64/go/bin/go.exe') build -o (Join-Path $destination 'php-worker.exe') (Join-Path $repository 'test/integration/php_worker_windows.go')
