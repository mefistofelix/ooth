...
`tmp/runtime-trueasync`; nothing is installed globally. The Windows server and
sockets extensions are included in that distribution and enabled by the test.

Windows, after building ooth:

```powershell
./test/integration/trueasync/prepare-windows.ps1
$env:CGO_ENABLED = '0'
$env:OOTH_TEST_TRUEASYNC = "$PWD/tmp/runtime-trueasync/windows/php.exe"
./build/windows-amd64/go/bin/go.exe run ./test/run.go test -v -run '^TestTrueAsyncCompatibility$' .
./build/windows-amd64/go/bin/go.exe run ./test/run.go test -v -run '^TestTrueAsyncExtraHandle$' .
```

Linux:

```sh
bash test/integration/trueasync/prepare-linux.sh
CGO_ENABLED=0 OOTH_TEST_TRUEASYNC="$PWD/tmp/runtime-trueasync/linux/php-trueasync-0.10.0-php8.6-linux-x86_64/php" \
  ./build/linux-amd64/go/bin/go run ./test/run.go test -v -run '^TestTrueAsyncCompatibility$' .
CGO_ENABLED=0 OOTH_TEST_TRUEASYNC="$PWD/tmp/runtime-trueasync/linux/php-trueasync-0.10.0-php8.6-linux-x86_64/php" \
  ./build/linux-amd64/go/bin/go run ./test/run.go test -v -run '^TestTrueAsyncExtraHandle$' .
```
