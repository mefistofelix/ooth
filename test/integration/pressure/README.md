...
```powershell
$env:CGO_ENABLED = '0'
$env:OOTH_TEST_PRESSURE = "$PWD/tmp/pressure-windows"
./build/windows-amd64/go/bin/go.exe run ./test/run.go test -run '^TestPressure(Proxies)?$' -count=2 -timeout 12m .
python test/integration/pressure/analyze.py tmp/pressure-windows --platform windows --output tmp/pressure-windows.csv
```

Linux, after setting the Linux runtime paths (do not inherit Windows paths):

```sh
export CGO_ENABLED=0 OOTH_TEST_PRESSURE="$PWD/tmp/pressure-linux"
./build/linux-amd64/go/bin/go run ./test/run.go test -run '^TestPressure(Proxies)?$' -count=2 -timeout 12m .
python3 test/integration/pressure/analyze.py tmp/pressure-linux --platform linux --output tmp/pressure-linux.csv
```

Use a writable delegated cgroup for whole-family supervision on Linux, as described in `../proxy/README.md` and implemented by `../with-cgroup.sh`. The ordinary build skips these opt-in experiments. No GitHub workflow is dispatched. Raw local artifacts belong under ignored `tmp/`; portable aggregate CSVs and the interpretation belong here.
