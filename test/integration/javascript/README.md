...
`tmp/runtime-js`. No global install or application dependency is added.
Supply Node from the existing Node probe preparation or a compatible runtime.

```powershell
python test/integration/javascript/prepare.py windows
$env:CGO_ENABLED = '0'
$env:OOTH_TEST_NODE = "$PWD/tmp/runtime-node/node-v26.10.0-win-x64/node.exe"
$env:OOTH_TEST_BUN = "$PWD/tmp/runtime-js/bun-windows-x64/bun-windows-x64/bun.exe"
$env:OOTH_TEST_DENO = "$PWD/tmp/runtime-js/deno-x86_64-pc-windows-msvc/deno.exe"
./build/windows-amd64/go/bin/go.exe run ./test/run.go test -v -count=1 -run 'Test(JavaScriptExtraHandle|NodeWorker|WorkerStopTransport|SocketHandoffConfig)' -timeout 90s .
```

```sh
python3 test/integration/javascript/prepare.py linux
export CGO_ENABLED=0
export OOTH_TEST_NODE="$PWD/tmp/runtime-node/node-v26.10.0-linux-x64/bin/node"
export OOTH_TEST_BUN="$PWD/tmp/runtime-js/bun-linux-x64/bun-linux-x64/bun"
export OOTH_TEST_DENO="$PWD/tmp/runtime-js/deno-x86_64-unknown-linux-gnu/deno"
./build/linux-amd64/go/bin/go run ./test/run.go test -v -count=1 -run 'Test(JavaScriptExtraHandle|NodeWorker|WorkerStopTransport|SocketHandoffConfig)' -timeout 90s .
```
