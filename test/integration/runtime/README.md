...
```powershell
$env:CGO_ENABLED = '0'
$env:OOTH_TEST_ARTIFACTS = "$PWD/tmp/runtime-matrix-windows"
./build/windows-amd64/go/bin/go.exe run ./test/run.go test -v -count=1 -run '^TestRuntimeLifecycle$' -timeout 240s .
```

```sh
export CGO_ENABLED=0
export OOTH_TEST_ARTIFACTS="$PWD/tmp/runtime-matrix-linux"
./build/linux-amd64/go/bin/go run ./test/run.go test -v -count=1 -run '^TestRuntimeLifecycle$' -timeout 240s .
```
...
`ooth.log` under ignored tmp/. All worker source, template and assertion code
