...
```sh
python3 test/integration/pid1.py
CGO_ENABLED=1 ./build/linux-amd64/go/bin/go run ./test/run.go test -race -timeout 60s .
```
...
```sh
sudo bash test/integration/with-cgroup.sh "$USER" env CGO_ENABLED=0 ./build/linux-amd64/go/bin/go run ./test/run.go test -timeout 90s .
sudo bash test/integration/with-cgroup.sh "$USER" env CGO_ENABLED=1 ./build/linux-amd64/go/bin/go run ./test/run.go test -race -timeout 90s .
sudo bash test/integration/with-cgroup.sh "$USER" --outside env CGO_ENABLED=0 ./build/linux-amd64/go/bin/go run ./test/run.go test -run TestCgroupPlacementFallback -v .
sudo env CGO_ENABLED=0 ./build/linux-amd64/go/bin/go run ./test/run.go test -run 'TestIdentityDifferentLinuxUser|TestSocketIdentityLinux' -v .
```
...
```powershell
& "$env:WINDIR/Microsoft.NET/Framework64/v4.0.30319/csc.exe" '/nologo' '/target:winexe' '/reference:System.Windows.Forms.dll' '/reference:System.Drawing.dll' "/out:$PWD\tmp\GuiProbe.exe" "$PWD\test\integration\GuiProbe.cs"
& ./build/windows-amd64/go/bin/go.exe run test/integration/probe_gui.go
& ./build/windows-amd64/go/bin/go.exe run ./test/run.go test -c -ldflags '-H=windowsgui' -o ../tmp/ooth-no-console.test.exe .
& ./build/windows-amd64/go/bin/go.exe run test/integration/probe_detached.go
```
...
& $stockGo build -o tmp/handoff-stock.exe test/integration/socket_handoff_windows.go
& ./build/windows-amd64/go/bin/go.exe build -o tmp/handoff-patched.exe test/integration/socket_handoff_windows.go
& ./tmp/handoff-stock.exe ./tmp/handoff-stock.exe
& ./tmp/handoff-patched.exe ./tmp/handoff-patched.exe
```
...
& ./tmp/handoff-stock.exe $node test/integration/node_handoff.cjs fd
& ./tmp/handoff-stock.exe $node test/integration/node_handoff.cjs stdin
$env:HANDOFF_RAW = '1'
try {
  & ./tmp/handoff-stock.exe $node test/integration/node_handoff.cjs native
} finally {
...
& ./tmp/handoff-stock.exe $node test/integration/node_handoff.cjs stdhandle
& ./tmp/handoff-stock.exe $node test/integration/node_handoff.cjs osfhandle
```
...
& ./build/windows-amd64/go/bin/go.exe run ./test/run.go test -run '^TestNodeWorker$' -v -timeout 30s .
```

On Linux the same fixture uses public `server.listen({fd: Number(process.env.OOTH_LISTEN_HANDLE)})`, without FFI or TCPWrap:

```sh
OOTH_TEST_NODE=/absolute/path/to/node ./build/linux-amd64/go/bin/go run ./test/run.go test -run '^TestNodeWorker$' -v -timeout 30s .
```

The lifecycle test passed locally on Windows amd64 and Linux amd64 in WSL, with Node 26.10.0. It is skipped unless `OOTH_TEST_NODE` is set. The runtime is a test tool, not an ooth dependency. Local portable archives were downloaded from nodejs.org into ignored `tmp/runtime-node/` and verified against the official `SHASUMS256.txt`; no system installation or CI trigger was changed. This original test is Node-only; the later common matrix also runs the shared worker with Bun and Deno on both OSes.
