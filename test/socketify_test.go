...
func TestSocketifyExtraHandle(t *testing.T) {
	...
	if python == "" {
		t.Skip("set OOTH_TEST_SOCKETIFY and prepare PYTHONPATH as documented in test/integration/socketify")
	}
	...
	for index := 0; index < 3; index++ {
		cmd := exec.CommandContext(ctx, python, filepath.Join("..", "test", "integration", "socketify", "worker.py"))
