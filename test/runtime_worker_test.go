		...
		for _, protocol := range protocols {
			for _, proxy := range []string{"direct", "caddy", "nginx"} {
				t.Run(name+"/"+protocol+"/"+proxy, func(t *testing.T) {
					...
					var command []string
					script, _ := filepath.Abs("../test/integration/node_worker.cjs")
					switch name {
					case "PYTHON":
						script, _ = filepath.Abs("../examples/worker.py")
					case "SOCKETIFY":
						script, _ = filepath.Abs("../test/integration/socketify/worker.py")
						workerEnv["TEST_OOTH_PROTOCOL"] = "1"
					case "TRUEASYNC":
						if runtime.GOOS != "windows" {
							t.Skip("official Linux TrueAsync 0.10.0 is static without FFI; user chose no rebuild")
						}
						script, _ = filepath.Abs("../test/integration/trueasync/worker.php")
