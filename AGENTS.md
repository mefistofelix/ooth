...
- Audit confirmed standard Cmd.Stdin inheritance works: the former syscall.StartProcess socket-inheritance override was removed and TCP/Unix lifecycle tests pass. Removing the shared-listener IOCP adaptation instead fails net.FileListener in Go workers, so it remains. Only the inherited AcceptEx fallback still checks deadline/close every 20 ms; distinguish that worker-side behavior from the supervisor's asynchronous AFD poller. Stock Process.Kill works; stock Windows Process.Signal(os.Interrupt) remains unsupported. Cmd.NewProcessGroup wraps existing SysProcAttr.CreationFlags plus hidden-console setup to keep main.go portable.
